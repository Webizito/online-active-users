...
if ( ! class_exists( 'webi_active_user' ) ) {
    class webi_active_user {
        ...
        public function wpoau_active_users_page() {
            echo '<div class="wrap"><h1>' . esc_html__( 'Online Users Lists', 'wp-online-active-users' ) . '</h1>';

            $users = $this->wpoau_online_users('array');

            if (empty($users)) {
                echo '<p>' . esc_html__( 'No users are online right now.', 'wp-online-active-users' ) . '</p></div>';
                return;
            }
        ...
        public function wpoau_modify_user_view( $views ) {
            ...
            $view = '<a href="' . esc_url( admin_url( 'users.php?page=wpoau-active-users' ) ) . '">' . esc_html__( 'User Online', 'wp-online-active-users' ) . ' <span class="count">(' . absint( $this->wpoau_online_users('count') ) . ')</span></a>';
        ...
        public function wpoau_support_and_faq_links( $links_array, $plugin_file_name, $plugin_data, $status ) {
            if ( strpos( $plugin_file_name, basename(__FILE__) ) ) {
                ...
                $links_array[] = '<a href="' . esc_url( 'https://wordpress.org/support/plugin/online-active-users/' ) . '" target="_blank">' . esc_html__( 'Support', 'wp-online-active-users' ) . '</a>';
                $links_array[] = '<a href="' . esc_url( 'https://webizito.com/wp-online-active-users/' ) . '" target="_blank">' . esc_html__( 'Docs', 'wp-online-active-users' ) . '</a>';
                $links_array[] = '<strong><a href="' . esc_url( 'https://wordpress.org/support/plugin/online-active-users/reviews/?rate=5#new-post' ) . '" target="_blank">' . esc_html__( 'Rate our plugin', 'wp-online-active-users' ) . '  <span style="color:#ffb900;font-size: 18px;position:relative;top:0.1em;">★★★★★</span></a></strong>';
            }
            return $links_array;
        }
        ...
        public function wpoau_plugin_by_link( $links ){
            ...
            $links[] = '<a href="' . esc_url( 'https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=APRNBJUZHRP7G' ) . '" target="_blank"><span style="font-weight: bold;">' . esc_html__( 'Donate', 'wp-online-active-users' ) . '</span></a>';
            $_link = '<a href="' . esc_url( $url ) . '" target="_blank">' . esc_html__( 'By', 'wp-online-active-users' ) . ' <span>Webizito</span></a>';
            $links[] = $_link;
            return $links;
        }
        ...
        public function wpoau_display_notice() {
            ...
            printf(
                ...
                esc_html__( 'Enjoying our WP Online Active Users plugin? Please consider leaving us a review %1$shere%2$s. Or support with a small donation %3$shere%2$s. We would greatly appreciate it!', 'wp-online-active-users' ),
...
if ( ! class_exists( 'Webi_Custom_Widget' ) ) {
    class Webi_Custom_Widget extends WP_Widget {
        ...
        public function __construct() {
            parent::__construct(
                'webi_custom_widget',
                __( 'WP Online Active User', 'wp-online-active-users' ),
                array( 'description' => __( 'Display Online Active Users.', 'wp-online-active-users' ), )
            );
        }
        ...
        public function widget( $args, $instance ) {
            ...
            if ( class_exists( 'webi_active_user' ) ) {
                ...
                echo '<p>' . esc_html__( 'Online Active Users:', 'wp-online-active-users' ) . ' <strong>' . absint( $active_users_count ) . '</strong></p>';
                echo '</div>';
            } else {
                echo '<p>' . esc_html__( 'WP Active User plugin not found.', 'wp-online-active-users' ) . '</p>';
            }
        ...
        public function form( $instance ) {
            $title = ! empty( $instance['title'] ) ? $instance['title'] : __( 'Active Users', 'wp-online-active-users' );
            ?>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_attr_e( 'Title:', 'wp-online-active-users' ); ?></label>
